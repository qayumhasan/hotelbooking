...
<script>
    ...
    document.querySelector('#add_to_grid').addEventListener('click', function(e) {
        ...
        if (getAllValue().itmeqty == '' || getAllValue().itmeqty == 0) {
            ...
            var html = '<tr class="insertItem"><th scope="row"><input type="hidden" name="order_id" value="{{rand(100,999)}}"><input type="text" name="item_name[]" value="%itemnameval%"/><input type="text" name="item_qty[]" value="%itemqty%"/><input type="text" name="item_unit[]" value="%itemunitval%"/>{{rand(100,999)}}</th><td>%itemname%</td><td>%qty%</td><td>%unitname%</td><td><span onclick="deleteItem(this)" class="deletebtn"><i class="fa fa-trash" aria-hidden="true"></i></span></td></tr>';
