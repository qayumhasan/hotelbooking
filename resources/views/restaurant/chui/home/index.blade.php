                  ...
                  <div class="row p-0">
                     ...
                     <div class="col-6">
                        <ul class="list-group pt-1 bg-menu">
                           ...
                           <li class="list-group-item bg-menu">
                              <a class="bg-menu" href="{{route('admin.checking.index',1)}}"><i class="fa fa-globe" aria-hidden="true"></i> History</a>
                           </li>

                           <li class="list-group-item bg-menu">
                              <a class="bg-menu" href=""><i class="fa fa-calendar-check" aria-hidden="true"></i> hfdsg
                              </a>
                           </li>
   ...
   document.querySelector('#addtogrid').addEventListener('click', function(e) {
      ...
      $.ajax({
         ...
         success: function(data) {
            ...
            var html = '<tr class="deleteddata"><th scope="row">%item_name%</th><td>%qty%</td><td>%rate%</td><td>%amount%</td><td><a data-whatever="'+data.id+'" onclick="edititem(this)"  class="badge bg-primary-light mr-2"><i class="la la-edit"></i></a><a data-whatever="'+data.id+'" class="badge bg-danger-light mr-2" onclick="deletitem(this)"><i class="la la-trash"></i></a></td></tr>';

            var newhtml = html.replace('%item_name%', data.item.item_name);
            var newhtml = newhtml.replace('%qty%', data.qty);
            var newhtml = newhtml.replace('%rate%', data.item.rate);
            var newhtml = newhtml.replace('%amount%', data.amount);
            document.querySelector('#kot_materails').insertAdjacentHTML('afterend', newhtml);
